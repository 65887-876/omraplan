...
const GuideForm = () => {
  ...
  try {
    ...
    const response = await axios.post('http://localhost:5000/api/auth/uploadguide', uploadData, {
      headers: {
  ...
  const validateFields = () => {
    const requiredFields = [
      'sex', 'city', 'services', 'languages', 'experience', 'description', 'dateFinPublication'
    ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/auth/guides', formData, {
        headers: {
      ...
      console.error('Error creating guide:', error.response); // Log the full error response
      alert('Échec de la création du guide. Veuillez réessayer.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-full" style={{ minHeight: `calc(100vh - ${headerHeight + bottomBarHeight}px)` }}>
          ...
          {step === 3 && (
            <div className="flex flex-col justify-center items-center sm:mt-10 w-full">
              <label className="sm:text-h3 text-h4 text-center font-medium py-8 text-neutral-13">Quel(s) service(s) proposez-vous ?</label>
              
              <div className="flex flex-wrap gap-2 h-[40px] justify-start items-center mb-4 w-full md:w-[800px]">
                {[
                  { name: 'Omra', description: '' },
                  { name: 'Visites des lieux', description: '' },
                  { name: 'Cours', description: '' },
                  { name: 'Omra Badel', description: 'Faire la omra pour un defun' },
                  { name: 'Handicape', description: 'Pousser un pelerins durant les rites de la omra' },
                  { name: 'Autres', description: '' },
                ].map(service => (
                  <label
                    key={service.name}
                    className={`flex justify-center items-center p-2 border rounded-full cursor-pointer ${formData.services.includes(service.name) ? 'bg-secondary-6 text-white' : 'bg-white text-neutral-13'} transition-colors duration-200`}
                  >
                    <input
                      type="checkbox"
                      name="services"
                      ...
                      className="hidden"
                    />
                    <span className="font-plus-jakarta font-medium text-sm">{service.name}</span>
                  </label>
                ))}
              </div>
              {formData.services.includes('Autres') && (
                <input
                  type="text"
                  name="otherServices"
                  value={formData.otherServices}
                  onChange={handleChange}
                  placeholder="Entrez ici le service"
                  className="w-full md:w-[800px] p-2 border border-gray-300 rounded-md mb-4"
                  style={{ borderRadius: '8px' }}
                />
              )}
              <div className="w-full md:w-[800px]">
                <p className="text-sm font-bold text-[#0A2A3D] mt-1">*Omra Badel : Faire la omra pour un defun</p>
                <p className="text-sm font-bold text-[#0A2A3D] mt-1">*Handicape : Pousser un pelerins durant les rites de la omra</p>
              </div>
            </div>
          )}
          {step === 4 && (
            <div className="flex flex-col justify-center items-center sm:mt-10 w-full">
              <label className="sm:text-h3 text-h4 text-center font-medium text-neutral-13">A propos de vous</label>

              <label className="sm:text-h6  text-left text-h6 font-medium text-neutral-13 mt-6">Quelles langues parlez-vous ?</label>
              ...
              <h1 className="sm:text-h6 text-left text-h6 py-4 font-medium text-neutral-13 mt-6">Nombre d’année d’experience en tant que guide ?</h1>
            ...
            <div className="flex justify-between items-center px-4 sm:px-8 md:px-16 pb-4 sm:pb-6 pt-7">
              ...
              {step === 8 && (
                <button
                  ...
                  disabled={loading}
                >
                  {loading ? 'Soumission...' : 'Créer Guide'}
                </button>
              )}
            </div>
          </div>

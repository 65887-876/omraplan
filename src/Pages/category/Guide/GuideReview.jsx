...
import { faMapMarkerAlt, faAt, faInfoCircle, faPhone, faSuitcase } from '@fortawesome/free-solid-svg-icons';
import { faWhatsapp } from '@fortawesome/free-brands-svg-icons';
import { RiUserVoiceFill } from 'react-icons/ri';

const LeftSection = ({ previewPhotos, settings, experience }) => {
  return (
    <div className="w-full md:w-[624px] h-[450px] bg-white p-8 rounded-2xl">
      <Slider {...settings}>
      ...
      <h1 className="text-h4 py-8 font-plus-jakarta font-bold text-neutral-13 text-left">Guide depuis {experience} ans</h1>
    </div>
  );
};
...
const RightSection = ({ formData, userData, showPhoneNumber, handleToggle }) => {
  ...
  return (
    <div className="w-full flex flex-col items-start p-8 gap-6 bg-white rounded-2xl overflow-hidden">
      ...
      <div className="flex items-start gap-2">
        <FontAwesomeIcon icon={faSuitcase} className="text-primary-6 mt-1" style={{ fontSize: '1.5em' }} />
        <div>
          <h4 className="font-plus-jakarta font-bold text-lg text-[#0A2A3D]">Expérience</h4>
          <p className="font-plus-jakarta text-sm text-[#0A2A3D]">{formData.experience}</p>
        </div>
      </div>
...
const GuideReview = ({ formData, previewPhotos, setFormData, showPhoneNumber, setShowPhoneNumber }) => {
  ...
  return (
    <div className="flex flex-col justify-between items-center mt-8 p-0 gap-4">
      ...
      <div className="flex flex-col md:flex-row justify-between gap-8 w-full max-w-7xl rounded-md bg-inherit">
        <LeftSection previewPhotos={previewPhotos} settings={settings} experience={formData.experience} />
        <RightSection
          formData={formData}
          userData={userData}

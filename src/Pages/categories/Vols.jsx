          ...
          {items.map((item, index) => (
            <li key={index} className="grid grid-cols-12 gap-4 items-center p-4 border-b border-gray-300" style={{ width: '1216px' }}>
              <div className="flex items-center col-span-3">
                ...
                <div className="ml-4">
                  <p className="font-bold font-plus-jakarta text-primary-10 text-[16px]">{item.compagnieAerienne || 'N/A'}</p>
                  <p className="text-sm font-plus-jakarta text-primary-6">{item.itineraire && item.itineraire.aller && item.itineraire.retour ? `${item.itineraire.aller.Aller} - ${item.itineraire.retour.Aller}` : 'N/A'}</p>
                </div>
              </div>

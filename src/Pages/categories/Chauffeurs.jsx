...
const Chauffeurs = ({ items, onDelete }) => {
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/auth/annonces/${id}`, {
          ...
          {items.map((item, index) => (
            <li key={index} className="grid grid-cols-12 gap-4 items-center p-4 border-b border-gray-300" style={{ width: '1216px' }}>
              <div className="flex items-center col-span-3">
                ...
                <div className="ml-4">
                  <p className="font-bold font-plus-jakarta text-primary-10 text-[16px]">{item.sex || 'N/A'}</p>
              ...
              <div className="flex justify-center col-span-2 ml-[15%]">
                ...
                <FaTrash className="text-primary-6 cursor-pointer ml-2" onClick={() => handleDelete(item._id)} />
              </div>
            </li>
          ))}
        </ul>

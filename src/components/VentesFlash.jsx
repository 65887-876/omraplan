...
// Example icon imports (replace with actual icons you're using)
...
const VenteFlash = () => {
  ...
  const sliderSettings = {
    ...
    slidesToScroll: 0.9,      // Scroll 1 card at a time
  ...
  const venteFlashAnnonces = categoriesWithIcons.reduce((acc, { name }) => {
    const categoryAnnonces = annonces?.[name]?.filter((item) => item.venteFlash) || [];
    return { ...acc, [name]: categoryAnnonces };
  }, {});

  const availableCategories = categoriesWithIcons.filter(({ name }) => venteFlashAnnonces[name]?.length > 0);
  ...
  const filteredAnnonces = activeCategory
    ? venteFlashAnnonces[activeCategory]
    : categoriesWithIcons.reduce((all, { name }) => [...all, ...venteFlashAnnonces[name]], []);
    ...
    <div className="relative vente-flash-container py-8 bg-yellow-50 md:px-20 overflow-visible">
        ...
        {filteredAnnonces.slice(currentIndex, currentIndex + 4).length > 0 ? (
          filteredAnnonces.slice(currentIndex, currentIndex + 4).map((item) => {
            const category = categoriesWithIcons.find(({ name }) =>
              venteFlashAnnonces[name]?.includes(item)
            )?.name;
      ...
      <div className="px-4">
        <button className="text-secondary-7 w-full  font-plus-jakarta  font-bold sm:w-[230px] h-[50px] bg-neutral-1 rounded-xl mt-8 mx-auto border border-secondary-7 hover:bg-secondary-4 hover:text-white transition duration-300 ease-in-out cursor-pointer">
          <a className="flex justify-center gap-2 items-center"><span>Voir toutes les ventes </span> <FaChevronRight className="mt-[1px]"/></a>
        </button>
      </div>
    </div>
  );

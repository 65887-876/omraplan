...
const BabySitterDetails = () => {
  ...
  useEffect(() => {
    const fetchBabySitter = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/auth/babysitters/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
        setBabysitter(response.data);
      } catch (error) {
        setError('Error fetching babysitter details');
      } finally {
        setLoading(false);
      }
    };
  ...
  const { photos, showNumber } = babysitter;
    ...
    <div className="bg-yellow-50 min-h-screen flex flex-col items-center px-4 py-16 relative">
      ...
      <div className='flex justify-start md:mr-[550px]'>
        <div className="flex text-sm items-center">
          ...
          <Link to="/babysitters" className="text-primary-4">BabySitters</Link>
          <span className="text-primary-4 mx-2">/</span>
      ...
      <div className="w-full max-w-3xl flex flex-col items-start p-8 gap-6 bg-white rounded-2xl overflow-hidden relative">
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="font-plus-jakarta text-sm text-[#0A2A3D]">{babysitter.city}</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {babysitter.otherLanguages && babysitter.otherLanguages.map((language, index) => (
                <span key={index} className="px-2 py-1 bg-gray-200 rounded-full text-sm text-[#0A2A3D]">
                  {language.trim()}
                </span>
              ))}
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="font-plus-jakarta text-sm text-[#0A2A3D] overflow-hidden">
              {babysitter.description.length > 100 ? babysitter.description.substring(0, 100) + '...' : babysitter.description}
            </p>
          </div>
        </div>
      </div>
